...
const right_floating = (<HTMLInputElement>document.getElementById("right-floating"));
const left = (<HTMLInputElement>document.getElementById("left"));
const left_floating = (<HTMLInputElement>document.getElementById("left-floating"));
...
function removeApplianceWidget(appliance: any) {
    ...
    if (appliances.length < 8) {
        (<HTMLInputElement>document.getElementById("newAppliance")!).disabled = false;
    }
};
...
async function addAppliance(new_appliance: any = null) {
    if (new_appliance == null) {
        const appliance_name = escapeHtml((<HTMLInputElement>document.getElementById("appName")!).value);
        const appliance_power = validateInt(escapeHtml((<HTMLInputElement>document.getElementById("appPower")!).value), 0, 20000);
        const appliance_hours = validateInt(escapeHtml((<HTMLInputElement>document.getElementById("appRunHours")!).value), 0, 16);
        const appliance_minutes = validateInt(escapeHtml((<HTMLInputElement>document.getElementById("appRunMinutes")!).value), 0, 59);
        ...
        if (appliances.length > 7) {
            (<HTMLInputElement>document.getElementById("newAppliance")!).disabled = true;
        }};
...
(async() => {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        (<HTMLInputElement>document.getElementById("region")!).value = getCookie("region", "A");
    });
    ...
    document.getElementById("region")!.addEventListener("change", async(event) => {
        region = (<HTMLInputElement>event.target!).value;
        setCookie("region", (<HTMLInputElement>event.target!).value, 365);
